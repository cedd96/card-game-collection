...
func _ready() -> void:
	...
	$MenuControl/ButtonBox/ButtonResume.grab_focus()

func _unhandled_input(event: InputEvent) -> void:
	if event.is_action_pressed("ui_cancel"):
		var _err = self.connect("tree_exited", Callable(get_parent(), "_on_PauseMenu_exited"))
...
func _on_ButtonResume_focus_entered() -> void:
	addFocusDecorations($MenuControl/ButtonBox/ButtonResume)

func _on_ButtonResume_focus_exited() -> void:
	for child in $MenuControl/ButtonBox.get_children():
		if not child is Button:
			child.call_deferred("queue_free")
	if $MenuControl/ButtonBox/ButtonResume != hoveredButton:
		$MenuControl/ButtonBox/ButtonResume.modulate = Color(1, 1, 1, 1)
		focusAnimationState = DARKEN

func _on_ButtonResume_mouse_entered() -> void:
	if $MenuControl.get_viewport().gui_get_focus_owner() != $MenuControl/ButtonBox/ButtonResume:
		hoveredButton = $MenuControl/ButtonBox/ButtonResume

func _on_ButtonResume_mouse_exited() -> void:
	hoveredButton = null
	if $MenuControl.get_viewport().gui_get_focus_owner() != $MenuControl/ButtonBox/ButtonResume:
		$MenuControl/ButtonBox/ButtonResume.modulate = Color(1, 1, 1, 1)
		hoverAnimationState = DARKEN

func _on_ButtonHowToPlay_pressed() -> void:
	if GlobalRules.selectedGame == "Schwimmen (31)":
		get_parent().add_child(load("res://Scenes/Menus/Pause/HowToPlay/PauseHowToPlaySchwimmen.tscn").instantiate())
	elif GlobalRules.selectedGame == "Mau-Mau":
		get_parent().add_child(load("res://Scenes/Menus/Pause/HowToPlay/pause_how_to_play_mau_mau.tscn").instantiate())
	call_deferred("queue_free")

func _on_ButtonHowToPlay_focus_entered() -> void:
	addFocusDecorations($MenuControl/ButtonBox/ButtonHowToPlay)

func _on_ButtonHowToPlay_focus_exited() -> void:
	for child in $MenuControl/ButtonBox.get_children():
		if not child is Button:
			child.call_deferred("queue_free")
	if $MenuControl/ButtonBox/ButtonHowToPlay != hoveredButton:
		$MenuControl/ButtonBox/ButtonHowToPlay.modulate = Color(1, 1, 1, 1)
		focusAnimationState = DARKEN

func _on_ButtonHowToPlay_mouse_entered() -> void:
	if $MenuControl.get_viewport().gui_get_focus_owner() != $MenuControl/ButtonBox/ButtonHowToPlay:
		hoveredButton = $MenuControl/ButtonBox/ButtonHowToPlay

func _on_ButtonHowToPlay_mouse_exited() -> void:
	hoveredButton = null
	if $MenuControl.get_viewport().gui_get_focus_owner() != $MenuControl/ButtonBox/ButtonHowToPlay:
		$MenuControl/ButtonBox/ButtonHowToPlay.modulate = Color(1, 1, 1, 1)
		hoverAnimationState = DARKEN
